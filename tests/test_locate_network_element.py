...
Asked which substations would lose supply if the line between Olsker and
Østerlars were out, the agent had no way to look that line up. It guessed
`element_index=11` — the Nexø–Bodilsker line — ran a perfectly good
...
TOPOLOGY = {
    "name": "test grid",
    "buses": [
        {"index": 0, "name": "Olsker", "vn_kv": 63.0, "in_service": True},
        {"index": 1, "name": "Østerlars", "vn_kv": 63.0, "in_service": True},
        {"index": 2, "name": "Allinge", "vn_kv": 63.0, "in_service": True},
        {"index": 3, "name": "Olsker 10 kV", "vn_kv": 10.0, "in_service": True},
        {"index": 4, "name": "Aux", "vn_kv": 63.0, "in_service": False},
        {"index": 5, "name": "Aux", "vn_kv": 63.0, "in_service": False},
    ],
    "branches": [
        {"index": 7, "name": "OLS-ØST | Olsker -> Østerlars [L7]", "from_bus": 0,
         "to_bus": 1, "kind": "line", "in_service": True},
        {"index": 6, "name": "OLS-ALL | Olsker -> Allinge [L6]", "from_bus": 0,
         "to_bus": 2, "kind": "line", "in_service": True},
        {"index": 11, "name": "BOD-NEX | Nexø -> Bodilsker [L11]", "from_bus": 1,
         "to_bus": 2, "kind": "line", "in_service": True},
        {"index": 3, "name": "Olsker Trf", "from_bus": 0, "to_bus": 3,
...
class TestResolvingAnElement:
    def test_a_named_bus_yields_the_index_of_every_branch_on_it(self):
        # The lookup the guessed index needed: Olsker → Østerlars is line 7,
        # and 7 is what `simulate_contingency` has to be called with.
        result = tools.locate_network_element("Olsker")
        assert result["bus_index"] == 0
        by_neighbour = {c["name"]: c for c in result["connected_to"]}
        assert by_neighbour["Østerlars"]["element_index"] == 7
        assert by_neighbour["Østerlars"]["element_type"] == "line"
        assert by_neighbour["Olsker 10 kV"]["element_type"] == "trafo"
    ...
    def test_the_index_is_named_as_the_other_tools_expect_it(self):
        ...
        (first, *_) = tools.locate_network_element("Olsker")["connected_to"]
    ...
    def test_a_named_branch_yields_its_index_and_endpoints(self):
        ...
        assert set(result["between"]) == {"Olsker", "Østerlars"}

    def test_matching_ignores_case(self):
        assert tools.locate_network_element("olsker")["bus_index"] == 0

    def test_a_bus_and_its_transformer_bus_are_not_confused(self):
        # "Olsker" and "Olsker 10 kV" are different buses; an exact match on
        # the shorter name must not sweep in the longer one.
        assert tools.locate_network_element("Olsker 10 kV")["bus_index"] == 3
...
class TestRefusingToGuess:
    def test_a_repeated_bus_name_is_refused_with_its_candidates(self):
        ...
        # `Værket A_aux`. Picking one is the error this tool exists to stop.
    ...
    def test_an_unknown_name_says_so_and_offers_the_real_ones(self):
        ...
        assert "error" in result
        assert "Olsker" in result["known_buses"]
    ...
    def test_a_backend_error_is_passed_through(self, monkeypatch):
        ...
        assert tools.locate_network_element("Olsker")["error"] == "backend down"
